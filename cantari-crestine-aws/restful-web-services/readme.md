...
### AWS configuration S3 Access for Static Content


Create EC2instance

Connect to ec2instance and install java
...
Connect to ec2
ssh -i "aws-cantari-crestine-ec2-key.pem" ec2-user@ec2-18-196-91-47.eu-central-1.compute.amazonaws.com
scp -i "aws-cantari-crestine-ec2-key.pem" cantari-crestine-server-1.0.jar  ec2-user@ec2-18-196-91-47.eu-central-1.compute.amazonaws.com:/tmp
...
java -jar cantari-crestine-server-1.0.jar 
-> the previous command will end after the session ends.
-> So that this to not happen:
nohup java -jar cantari-crestine-server-1.0.jar &
    -> nohup = no hang up = even if the session ends, the application will continue to run
```
{
    "Version": "2012-10-17",
    "Statement": [
